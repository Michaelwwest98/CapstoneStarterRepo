...
7. The RAK811 Breakout Board must be connected to the computing subsystem in order to transmit data from the computing subsystem to the user interface subsystem. In order connect the RAK811 Breakout Board and the computing subsystem, a RAKDAP1 is used to connect the transceiver to the computing subsystem. The RAKDAP1 is ewuipped with a USB that enables it to connect to any device that possess a USB port, which the computing subsystem has. Before you are able to do this however, you must connect the RAKDAP1 with the RAK811 attached to it to a device that can access the internet. From there you must register and configure the device to how the user wants it using a program given by the manufacturer. Once the setup is complete, the RAKDAP1 with the RAK811 can be connected to the computing subsystem and be operated using commands found in a python library which the computing system will be utilizing.
